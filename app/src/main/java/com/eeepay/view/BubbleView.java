...
import com.eeepay.utils.ABPixelUtil;
...
public class BubbleView extends View {
    ...
    public BubbleView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mBottleWidth = ABPixelUtil.dp2px(130, context);
        mBottleHeight = ABPixelUtil.dp2px(260, context);
        mBottleBorder = ABPixelUtil.dp2px(8, context);
        mBottleRadius = ABPixelUtil.dp2px(15, context);
        mBottleCapRadius = ABPixelUtil.dp2px(5, context);

        mWaterHeight = ABPixelUtil.dp2px(240, context);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        float bt = mContentRectF.centerX() - mBottleHeight / 2;
        float br = mContentRectF.centerX() + mBottleWidth / 2;
        float bb = mContentRectF.centerX() + mBottleHeight / 2;
        /** 画瓶身 **/
        mBottlePath.reset();
        mBottlePath.moveTo(bl - mBottleCapRadius, bt - mBottleCapRadius);
        mBottlePath.quadTo(bl, bt - mBottleCapRadius, bl, bt);//贝塞尔曲线 三点确定一个弧线
